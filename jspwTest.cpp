...
void solve() {
    ...
    int ans = 0;
    read(n,m,x);
    map<char, vt<pii>>layout;
    FOR(i, n){
        string s;
        cin >> s;
        FOR(j, m){
            layout[s[j]].pb(mp(j,i));
        }
    }
    int q;
    read(q);
    ...
    FOR(i, 1, q){
        ...
        FOR(j, sza(layout['S'])){
            dbg(layout);
            int dy = abs(layout['S'][j].se-key[0].se);
            int dx = abs(layout['S'][j].fi-key[0].fi);
            if(sqrt(dx*dx+dy*dy) <=x){
                sh = 0;
                break;
            }
        }
