...
// read tutorial after trying to use nseries but forgot that the behaviour gets weird everytime 
// im trying to sum up to n where n is divisible by 10 since it resets to 1
// all i had to do was precompute a table using bruteforce and just lookup the answers
// s(n) = sumdigits(n) + s(n-1) bruh
vt<ll>s(MX);
