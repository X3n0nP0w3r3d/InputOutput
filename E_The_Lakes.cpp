...
// simple bfs problem
ll total = 0;
ll bfs(int a, int b, vt<vt<int>>& grid, ll& res){
    if(grid[a][b]==0) return 0;
    res+=grid[a][b];
    grid[a][b] = 0;
    if(a+1 <sza(grid) && grid[a+1][b] > 0) bfs(a+1, b, grid, res);
    if(b+1 <sza(grid[0]) && grid[a][b+1] > 0) bfs(a, b+1, grid, res);
    if(a-1 >-1 && grid[a-1][b] > 0) bfs(a-1, b, grid, res);
    if(b-1 > -1 && grid[a][b-1] > 0) bfs(a, b-1, grid, res);
    return res;
}
...
void solve() {
    ...
    FOR(i, n)FOR(j, m){
        if(grid[i][j] > 0){
            total = 0;
            ans = max(ans, bfs(i, j, grid, total));
        }
    }
    cout << ans << "\n";
}
