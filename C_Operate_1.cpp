...
void solve() {
    ...
    string s, t;
    read(k, s, t);
    // multiset<char>tgt(all(t));
    // int mcount = (sza(s) > sza(t) ? sza(t)-sza(s) : 0);
    // FOR(sza(s)){
    //     auto it = tgt.find(s[i]);
    //     if(it!=tgt.end()){
    //         mcount++;
    //         tgt.erase(it);
    //     }
    // }
    // dbg(mcount, sza(t));
    // atynw(abs(sza(t)-mcount)<=k);
    if(abs(sza(s)-sza(t)) > 1)atynw(0);
    bool used = abs(sza(s)-sza(t));
    FOR(sza(t)){
        if(s[i]!=t[i]){
            if(used){
                atynw(0);
                return;
            } else if(s.substr(i+1, sza(s)-1-i)==t.substr(i, sza(t)-1-i)){
                used = 1;
