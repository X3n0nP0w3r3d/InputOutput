...
template<typename A, typename B> ostream& operator<<(ostream &os, pair<A, B> const &p) { return os << PAIR_LEFT << p.f << PAIR_SEP << p.s << PAIR_RIGHT; }
template<typename T> typename enable_if<is_iterable<T>::value&&!is_same<T, string>::value,ostream&>::type operator<<(ostream &os, T const &v) {
    os << IT_LEFT; 
...
// better solution is to reverse sort and keep subtracting values from k till negative
// or if k isn't negative by then just print k since thats the minimum required coins to grab
void solveJ(){
    int n,k;
    read(n,k);
    vt<int> l(n);
