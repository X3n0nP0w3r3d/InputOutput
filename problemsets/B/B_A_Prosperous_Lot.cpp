...
/** AC 1st attempt lol how is this 1200
 * also was unsure about the output since i was thinking i could just pad any amount of 0s infront but luckily i didn't since it wanted a positive INTEGER and not a string
 * Constraints were that the output is no greater than 10^18 so we can only have a max of 18 digits
 * Since 8's have two loops we can have k up to 36 anything beyond is impossible
 * Add the digit 8 floor(n/2) times
 * If k is odd then just add the digit 4 on to the end of the number
 */
void solve() {
    // #ifndef LOCAL
...
// fun few lines solve version
void solvefewlines(){
    int k;
    cin >> k;
    if(k>36) cout << -1;
    else cout << string(k/2, '8') << string(k&1, '4');
}
