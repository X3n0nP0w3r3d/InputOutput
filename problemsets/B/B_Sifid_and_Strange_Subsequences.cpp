...
/**
 * Key points:
 * We can prove that the largest subsequence contains at most 1 positive integer
 * For every integer less than or equal to 0 we can add that to the subsequence
 * We can only add a positive integer x to the subsequence given that the difference of the sequence of 0 or less has a min distance greater than x for all pairs
 * We can find this out by sorting then finding the min dist between each value (sorted will make this easier since least diff between each value)
 * If there exists such x then print it out
 * Last condition needed a check so that out of bounds error doesn't occur i.e. all values are 0 or less and then it tries to get a value at n+1 (0 indexed);
 */
void solve() {
    // #ifndef LOCAL
    ...
    sort(all(nums));
    FOR(i, 1, ans){
        mn_diff = min(mn_diff, abs(nums[i]-nums[i-1]));
    }
    if(ans < n && nums[ans] <= mn_diff && nums[ans] > 0)ans++;
    cout << ans << "\n";
}
