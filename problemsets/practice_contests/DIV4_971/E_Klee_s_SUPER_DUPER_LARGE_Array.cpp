...
// AC YESSSSSS
/** Had to deal with things being handled as ints and usign infinity since my INF macro is for integers and the min value can be long long
 * Did a natural series summation and accounted for start values
 * Did binary search to find minimum, limiting to left side if difference between prefix and suffix sum was greater than 0
 * otherwise go right side
 * update minimum on each search or terminate if diff is 0 since that is lowest abs diff
 */
void solve() {
    // #ifndef LOCAL
    ...
    while(l<=r){
        ...
        // if(mid==lm)break;
        // lm = mid;
        ll prf = nseries(mid)-nseries(k-1);
        ll sf = ((nseries(n+(k-1))-nseries(k-1))-prf);
        if(prf-sf==0){
            cout << 0 << "\n";
            return;
        }
        minV = min(minV, abs(prf-sf));
        if(prf-sf > 0){
            r=mid-1;
        } else {
            l=mid+1;
