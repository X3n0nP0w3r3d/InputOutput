...
void solve() {
    ...
    read(n);
    int ans = 0;
    for(int i=2;1ll*i*i<=n;i++)if(valid[i])ans++;
    cout << ans;
}
...
int main() {
    ...
    // cin >> tc;
    //prims.eb(2);
    //for(int i=4; i<sqrt(1e9); i+=2)prime[i]=1;
    for(int i=2; i<2e6+10; i++){
        if(!prime[i]){
            prims.pb(i);
            for(int j=i+i; j<=2e6+10;j+=i)prime[j]=1;
        }
    }
    for(auto x : prims)for(auto y : prims){
        if(x*y>=(2e6+10))break;
        if(x!=y)valid[x*y]=1;
    }
    for(auto i : prims){
        if(1ll*i*i*i*i>=(2e6+10))break;
        valid[i*i*i*i]=1;
    }
    for (int t = 1; t <= tc; t++) {
        // cout << "Case #" << t << ": ";
