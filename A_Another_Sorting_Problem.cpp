...
bool csort(pair<int, string>&a , pair<int, string>&b){
    int i = 0;
    if(a.se == b.se)return true;
    while(a.se[i]==b.se[i])i++;
    dbg(a.se[i], b.se[i]);
    if(a.se[i] < b.se[i]) return !(i%2);
    else return i%2;
}

// AC LFGGGGGGG
/** thank c++ custom sorting functions lol, used array of pairs with index as first value and the string as second value
 * used custom comparator that has an index value that increases everytime a character in string a is the same as string b at index i;
 * if both strings are equal then skip the checks and return true (no sorting needed) [this is literally useless since the problem statement says they're unique pairwise lmao]
 * checks if character ai is less than bi and then return the not value of i%2 (flips result depending if even index or not [epic]) [descending order for even and ascending for odd]
*/
void solve() {
    // #ifndef LOCAL
