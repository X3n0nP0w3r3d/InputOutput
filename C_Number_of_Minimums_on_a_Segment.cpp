...
struct segtree {
    ...
    void init(int n){
        size = 1;
        while(size<n)size*=2;
        tree.assign(size*2, mp(INF, 0));
    }
    void build(vt<pii>&a, int x, int lx, int rx){
        if(rx-lx==1){
            if(lx<sza(a))tree[x]=a[lx];
            return;
        }
        int mid = (rx+lx)/2;
    ...
    void set(int i, int v, int x, int lx, int rx){
        ...
        if(tree[x*2+1].fi < tree[x*2+2].fi){
            tree[x] = tree[x*2+1];
        } else if (tree[x*2+2].fi > tree[x*2+1].fi){
            tree[x] = tree[x*2+2];
        } else {
            tree[x] = mp(tree[x*2+1].fi, tree[x*2+2].se + tree[x*2+1].se);
        }
    }
