 ...
 * 
 * When reversing substrings, the kth character becomes the next part of the prefix of the answer, formally the prefix is: sK, sK-1, s1, ...
 * We can observe that there are n-k+1 substrings to reverse so the prefix of the answer up to i is sK, sK+1, ..., sK+(n-k+1)-1
 * We can also notice that the prefix substring is basically the suffix of the original substring (sK...sN)
 * The suffix of the answer can then be observed as s1...sK-1 if n and k are of the same parity (n%2==k%2)
 * and sK-1...s1 otherwise
 */

string rstr(string &s, int n, int k){
    string pre = s.substr(k-1, n-k+1);
    string suff = s.substr(0, k-1);
    if(n%2==k%2)reverse(all(suff));
    return pre + suff;
}
