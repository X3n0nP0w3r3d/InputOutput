...
// main takeaways:
/*
    - read question properly (only uppercase characters required two handing)
    - hypot(x, y) is in standard library for calculating size of hypotenuse (cool)
    - 
*/
void solve() {
    // #ifndef LOCAL
    ...
    int ans = 0;
    read(n,m,x);
    vt<pair<char, pii>>keys;
    map<char, bool>enabled;
    vt<pii>shifts;
    map<char, bool>dist;
    FOR(i, n){
        string s;
        ...
        FOR(j, m){
            ...
            keys.pb(mp(s[j], mp(j, i)));
            dist[s[j]]=0;
            enabled[s[j]]=1;
        }
    }
    FOR(i, sza(keys))FOR(j, sza(shifts)){
        dist[keys[i].fi] = dist[keys[i].fi] || hypot(shifts[j].fi-keys[i].se.fi, shifts[j].se-keys[i].se.se) <= x;
    }
    int q;
    read(q);
    string txt;
    cin >> txt;
    FOR(q){
        if(!enabled[tolower(txt[i])] || (!sza(shifts) && txt[i]-'a' < 0)){
            cout << "-1";
            return;
        }
        if(!dist[tolower(txt[i])] && txt[i]==toupper(txt[i])){
            ans++;
        }
    }
    cout << ans;
