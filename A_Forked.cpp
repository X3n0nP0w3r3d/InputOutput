...
/**
 * Kept getting wrong answer because I didn't use 2 sets wtf
 * I assumed i could just leave out duplicate elements and assume that the size of the set would be 14-16 but oh boy that did not work out
 * spent over an hour figuring out useless shit!!!! i just had to use two separate sets and find matching values WTF
 */
void solve() {
    // #ifndef LOCAL
    ...
    int a,b,xk,yk,xq,yq;
    int ans = 0;
    read(a,b,xk,yk,xq,yq);
    set<pii> sq;
    set<pii> sq2;
    sq.insert(mp(xk+a, yk+b));
    sq.insert(mp(xk+a, yk-b));
    sq.insert(mp(xk-a, yk+b));
    sq.insert(mp(xk-a, yk-b));

    sq.insert(mp(xk-b, yk+a));
    sq.insert(mp(xk+b, yk+a));
    sq.insert(mp(xk-b, yk-a));
    sq.insert(mp(xk+b, yk-a));
    
    sq2.insert(mp(xq+a, yq+b));
    sq2.insert(mp(xq+a, yq-b));
    sq2.insert(mp(xq-a, yq+b));
    sq2.insert(mp(xq-a, yq-b));

    sq2.insert(mp(xq-b, yq+a));
    sq2.insert(mp(xq+b, yq+a));
    sq2.insert(mp(xq-b, yq-a));
    sq2.insert(mp(xq+b, yq-a));
    for(auto i : sq){
        if(sq2.find(i) != sq2.end())ans++;
    }
    cout << ans << "\n";
    dbg(sq);
}
