...
void solve() {
    ...
    /** gfg solution very simple and clean
     * observe that there's 2^n different possible states for n bits
     * loop through 0 to (2^n)-1 and initialise bitset of 32 bits (n<=32 ??)
     * notice that resulting gray code is the result of xor operation of i and LSR (i >> 1) of i by 1
     * convert bitset to string using .to_string() method of bitset and use substr() to output only n bits
     **/

    int n;
    read(n);
    FOR(1 << n){
        bitset<32>bits(i ^ (i>>1));
