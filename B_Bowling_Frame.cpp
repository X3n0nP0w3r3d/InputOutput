...
// AC - read editorial though since I was doubting validity of my initial thoughts (using nseries)
// appparently it can be proven that an arrangement can be made in any way given that the sum of w+b satisfies a natural series summation
// I was thinking about that then thought what if there were differing amounts of pins and that they couldn't mix etc. but apparently that is not the case for the max
// side width
// other solutions use less memory by checking the nseries sum with a binary search instead of generating the series sums and using lower_bound() [literally bsearch with extra steps and memory]
void solve() {
    // #ifndef LOCAL
    ...
    ll w,b;
    read(w,b);
    auto ind = lower_bound(all(sw), w+b);
    if(*ind > w+b)ind--;
    cout << ind-sw.begin() << "\n";
}
