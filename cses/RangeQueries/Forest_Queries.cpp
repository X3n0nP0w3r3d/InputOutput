...
// ac first try lfggggg
// 2d psa uses idea that in pos x,y is the sum of all numbers from 0,0 to x,y
// if L is not 0,0 then subtract rectangles before and above the region we want
// make sure to account for double subtraction and we can print answer

// construct 2d psa using psa[y+1][x] + psa[y][x+1] - psa[y][x] + grid[y][x]
// remember psa always starts with 0 value to prevent indexing issues when it comes to 1 indexing (i think)
...
void solve() {
    ...
    FOR(i, n)FOR(j, n){
        a[i+1][j+1] = (int)(grid[i][j]=='*') + a[i+1][j] + a[i][j+1] - a[i][j];
    }
    FOR(q){
        int y1,x1,y2,x2;
        read(y1,x1,y2,x2);
