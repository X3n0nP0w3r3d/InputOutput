...
// ac after 3 attempts
// was using it->se!i+1 which is wrong since the array is sorted so i have to use pair index instead (a[i].se)
// after this change was made it ac'd
...
void solve() {
   ...
   sort(all(a));
   FOR(n){
    auto it = lower_bound(all(a), mp(x-(a[i].fi), 0));
    if(it->se!=a[i].se && (it->fi + a[i].fi) == x){
        cout << (*it).se << " " << a[i].se;
        return;
    }
   }
