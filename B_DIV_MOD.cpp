...
// ACTUALLY AC'D LOL idk how (i actually do kind of?)
// did it in like 5 mins
/** key ideas
 * if the largest value R is less than constant A, then the remainder is equal to R which is maximal
 * otherwise initialise answer with R as x in ans= x/a + x%a
 * check if r-l is greater than r%a because then we can get the maximum remainder by subtracting (r%a)+1
 * find max of this result and current initial result
 */
...
void solve() {
    ...
    if(r<a){
        ...
        int res = (r/a) + r%a;
        if(r-l > r%a)res=max(res, ((r-(r%a)-1)/a) + (r-(r%a)-1)%a);
        cout << res << "\n";
    }
}
...
/**
 * jiangly soln
 * initialise ans by implementing formula and using r as largest x
 * initiallise var v with r+1 divided by a multiplied all by a then subtract 1
 * compare var v to initial ans and print max of the two
void solve() {
    int l, r, a;
    std::cin >> l >> r >> a;
