...
/** Insights
 * O(k) runtime
 * Since we can always make a composite number out of the given digits:
 * If the number contains a digit corresponding to 1,4,6,8 or 9 then that digit is an answer itself
 * The maximum optimal amount of digits is 2 (when there are no composite digits)
 * If the there are no composite digits then loop through all digits twice, one for the beginning digit and one for the second digit
 * Check if composite and if it is then print the number out
 */
void solve() {
    // #ifndef LOCAL
