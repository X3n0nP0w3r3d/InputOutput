...
/**  required reading editorial to confirm an idea that counting characters was required but not directly. The keyword that n was "odd" was vital here. 
 * That means for each position the odd count character will always be the missing string
 * We can figure out the missing characters by initialising an empty char array and xor'ing each position in the array with the original strings and modified strings
 * this essentially reverses the xor of each remaining string and leaves the excluded one remaining and then we can print out all characters within the range of m characters
 * hence giving the answer
*/
void solve() {
    vt<char> ans(1e6+10, 0);
    int n,m;
    read(n,m);
    n=(n*2)-1;
    FOR(i, 1, n+1){
