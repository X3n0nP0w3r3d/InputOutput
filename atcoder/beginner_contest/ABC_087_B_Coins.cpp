...
void solveGPT() {
    ...
    // Iterate through possible counts of 500-yen coins
    for (int i = 0; i <= A; ++i) {
        // Iterate through possible counts of 100-yen coins
        for (int j = 0; j <= B; ++j) {
            int remaining = X - (500 * i + 100 * j);

            // Check if the remaining amount can be exactly made using 50-yen coins
            if (remaining >= 0 && remaining % 50 == 0) {
                int k = remaining / 50;
                if (k <= C) {
                    ++ways; // Valid combination
                }
            }
        }
    }
