...
// AC
/**
 * Took 1 rewrite to get it right - I need to be more careful with proofreading my code idk if im tired or smthn
 * but decreasing both l and r instead of one being increase and the other decrease is being braindead
 */
void solve() {
    // #ifndef LOCAL
    ...
    while(start<sza(s)){
        cnt=1;
        int o = find(s.begin()+start, s.end(), '/')-s.begin();
        if(o>0 && o<n-1){
            int l=o-1,r=o+1;
            while(r<n && l>=0 && s[r]=='2' && s[l]=='1'){
                cnt+=2;
                r++;
                l--;
            }
