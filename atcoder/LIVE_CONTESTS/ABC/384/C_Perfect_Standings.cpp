...
// }
bool lex(pair<int,char>&a,pair<int,char>&b){
    if(a.first > b.first){
        return true;
...
bool lexs(pair<int,string>&a,pair<int,string>&b){
    if(a.first > b.first){
        return true;
    } else if(a.first==b.first){
        if(a.second > b.second)return false;
...
void solve() {
    ...
    // #endif
    int a,b,c,d,e;
    vt<pair<int, char>> q = {{0,'A'},{0,'B'},{0, 'C'},{0,'D'},{0,'E'}};
    vt<pair<int,string>>scores;
    vt<string> final;
    vt<int> r(5);
    FOR(5){
        int k;
    ...
    cout << "ABCDE\n";
    FOR(i,5)FOR(j,5)FOR(k,5)FOR(l,5){
        int d=1;
        pair<int,string>res;
        res.second+=q[i].second;
        res.first+=q[i].first;
        res.second+=q[j].second;
        res.first+=q[j].first;
        res.second+=q[k].second;
        res.first+=q[k].first;
        res.second+=q[l].second;
        res.first+=q[l].first;
        FOR(_, 5){
            if(count(all(res.second), q[_].second) > 1) d = 0;
        }
        if(d)scores.eb(res);
    }
    FOR(i,5)FOR(j,5)FOR(k,5){
        int d=1;
        pair<int,string>res;
        res.second+=q[i].second;
        res.first+=q[i].first;
    ...
    //dbg(scores);
    sort(all(scores), lexs);
    for(auto i : scores){
        string s = i.second;
        sort(all(s));
        mkuniq(s);
        if(find(all(final), s)==final.end())final.eb(s);
    }
    sort(all(q), lex);
    for(auto i : final){
        while(count(all(final), i) > 1){
            final.erase(
                find(
                    find(all(final), i), final.end(), i
                    )
                );
        }
    }
 ...
 * VEC(ll, A, 5);
  ...
  sort(all(dat), [&](auto& L, auto& R) -> bool {
    if (L.fi != R.fi) return L.fi > R.fi;
    return L.se < R.se;
  });
  for (auto& [a, b]: dat) print(b);
 */
