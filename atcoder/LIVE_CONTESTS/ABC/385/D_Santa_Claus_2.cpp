...
void mmove(ll& x, ll& y, ll dx, ll dy, ll mult, set<pii>h){
    ll oX=x,oY=y;
    y+=dy*mult;
    x+=dx*mult;
}
...
void solve() {
    ...
    FOR(m){
        ...
        cin >> ins >> c;
        dbg(cX, cY, ins, c);
        if(ins=='U')mmove(cX, cY, 0LL, 1LL, c, h);
        if(ins=='L')mmove(cX, cY, -1LL, 0LL, c, h);
        if(ins=='D')mmove(cX, cY, 0LL, -1LL, c, h);
        if(ins=='R')mmove(cX, cY, 1LL, 0LL, c, h);
        if(h.count(mp(cX,cY)))visited.insert(to_string(cX)+","+to_string(cY));
    }
    
    cout << cX <<" "<< cY << " " << sza(visited);
}
