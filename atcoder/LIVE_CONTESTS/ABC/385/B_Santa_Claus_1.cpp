...
void mmove(vt<string>& grid, int h, int w, int& x, int& y, int dx, int dy){
    if(x+dx > w-1) return;
    if(y+dy > h-1) return;
    y+=dy;
    x+=dx;
...
void solve() {
    ...
    int cnt = 0;
    for(auto c : moves){
        if(c=='U')mmove(s,h,w,cX,cY,0,-1);
        if(c=='L')mmove(s,h,w,cX,cY,-1,0);
        if(c=='D')mmove(s,h,w,cX,cY,0,1);
        if(c=='R')mmove(s,h,w,cX,cY,1,0);
        if(s[cY][cX]=='@'){
            visited.insert(to_string(cY)+","+to_string(cX));
        }
    }
    cout << cY+1 <<" "<< cX+1 << " " << sza(visited);
}
