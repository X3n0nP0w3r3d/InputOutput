...
// AC lfg
/** noticed that just implementing the problem was gonna TLE, n,m <=10^18 lol good luck running that within a second xd
 * realised that i can just implement the same conditions but use mod instead to remove unnecessary steps because n wouldn't be more than 2*m 
 * after some arbitrary number of 2*m removals so just get the mod
 * do the same for m, 2*n etc.
 * ez profit
 */
void solve() {
    // #ifndef LOCAL
