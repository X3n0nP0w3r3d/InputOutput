...
void solve() {
    ...
    //kotatsugame solution https://codeforces.com/contest/2051/submission/297846253
    int n,k;
    cin>>n>>k;
		vector<pair<int,bool> >ab;
		ab.reserve(n+n);
		for(int i=0;i<n;i++)
		{
			int a;cin>>a;
		...
		{
			int x=ab[i].first;
			if(ac<=k)ans=max(ans,(long long)x*bc);
